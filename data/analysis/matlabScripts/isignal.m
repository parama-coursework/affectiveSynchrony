function pY=isignal(DataMatrix,xcenter,xrange,sm,sw,em,dm,rm,s1,s2)
% Y=isignal(DataMatrix,xcenter,xrange,SmoothMode,SmoothWidth,ends,...
% DerivativeMode,Sharpen,Sharp1,Sharp2)
...
% treatment, the derivative order (0th through 4th), and peak 
...
% Version 1.92: Derivatives computed with respect to x, corrected for 
% non-uniform x axis intervals. Minor bug fixes and display improvements.
...
% remove stray data points). The Tab kay cancels all settings and restores
% the original signal.  The O (letter O) key saves the processed signal as
% a "mat" file, in a location and with a file name that you specifiy.
%
...
global X Y xo dx DerivativeMode Sharpen Sharp1 Sharp2 SmoothWidth
global SmoothType ends SmoothMode SavedSignal PeakLabels Report autozero
format short g
format compact
warning off all
switch nargin % Process arguments
    ...
    case 2
      ...
      if isscalar(xcenter) % if second argument is scalar
         % Maaaazust be isignal(DataMatrix,xcenter)
      ...
      SmoothMode=0; % Initial SmoothMode = None
      SmoothWidth=3; % Initial smooth width
      SmoothType='Rect.';  % Label for initial SmoothMode
      DerivativeMode=0; % Derivative mode off initially
      ends=0;  % Initial smooth ends setting zero (no tapering)
    ...
    otherwise
      ...
      disp('isignal(DataMatrix,xcenter,xrange,SmoothMode,SmoothWidth,ends,DerivativeMode,Sharpen,Sharp1,Sharp2); ')
      beep
      return
end % switch nargin
...
% Plot the signal
pY=ProcessSignal(X,SavedSignal,DerivativeMode,SmoothWidth,SmoothMode,ends,Sharpen,Sharp1,Sharp2);
Y=pY;
...
global X Y xx yy xo dx SmoothMode SmoothWidth DerivativeMode
global Sharpen Sharp1 Sharp2 SavedSignal SavedBackground
global SmoothType ends PeakLabels Overlay Report GaussEstimate
...
if isscalar(key),
  ...
  switch double(key),
    ...
    case 8
        ...
        Y=tempY;
        SavedSignal=Y;
        Y=ProcessSignal(X,SavedSignal,DerivativeMode,SmoothWidth,SmoothMode,ends,Sharpen,Sharp1,Sharp2);
        [xx,yy]=RedrawSignal(X,Y,xo,dx);  
    case 92
    ...
    case 120 % When 'x' key is pressed, toggles between ends 0 and 1
         ...
         [xx,yy]=RedrawSignal(X,Y,xo,dx);    
    case 100
        % When 'd' key is pressed, steps through DerivativeModes 0->4->0
        ...
        if DerivativeMode==5,DerivativeMode=0; end
        Y=ProcessSignal(X,SavedSignal,DerivativeMode,SmoothWidth,SmoothMode,ends,Sharpen,Sharp1,Sharp2);
        [xx,yy]=RedrawSignal(X,Y,xo,dx);    
      ...
      case 103
          ...
          [xx,yy]=RedrawSignal(X,Y,xo,dx);
      case 98
          % When 'c' key is pressed, decreases Sharp2
          if Sharpen==0,Sharpen=1;end
          Sharp2=Sharp2-.1*Sharp2;
    ...
    case 9 % When 'Tab' key is pressed, resets to original signal and modes
         ...
         DerivativeMode=0;
         Sharpen=0;
         Y=ProcessSignal(X,Y,DerivativeMode,SmoothWidth,SmoothMode,ends,Sharpen,Sharp1,Sharp2);
         [xx,yy]=RedrawSignal(X,Y,xo,dx);   
    ...
    case 45
         % When '-' key is pressed, invert the signal
          ...
          [xx,yy]=RedrawSignal(X,Y,xo,dx); 
    case 105
         % When 'i' key is pressed, integrates the signal
          sum=0;
         for n=1:length(X),
              sum=sum+Y(n);
              Y(n)=sum;
    ...
    case 113
      ...
      case 107
        ...
        disp('KEYBOARD CONTROLS, version 1.9:')
        ...
        disp(' Cycle smooth types..........S (None, Rectanglular, Triangle, Gaussian, Savitzky-Golay)')
        disp(' Toggle smooth ends..........X (0=ends zeroed  1=ends smoothed (slower)')        
        disp(' Cycle derivative orders.....D (Steps through 0th to 4th order)')
        ...
        disp(' Toggle overlay mode.........L Overlays original signal in upper window')
        ...
        disp(' Save output to disk.........O as .mat file containing processed signal') 
      case 119
        ...
        if lastpoint<1,lastpoint=1;end
          disp(['isignal(DataMatrix,'  num2str(X(round(xo))) ',' num2str(X(round(firstpoint))-X(round(lastpoint)))  ',' num2str(SmoothMode)  ',' num2str(SmoothWidth) ',' num2str(ends) ',' num2str(DerivativeMode)  ',' num2str(Sharpen)  ',' num2str(Sharp1)  ',' num2str(Sharp2) ');' ] )
      case 111
        % When 'o' key is pressed, processed signal is saved as Output in mat file
        SavedY=Y;
        datasize=size(SavedY);
        if datasize(1)<datasize(2),SavedY=Y';end
        Output=[SavedY];
        uisave('Output');
      otherwise  
...
global DerivativeMode PeakLabels Overlay Report autozero
global SavedSignal GaussEstimate LorentzEstimate logymode
...
if autozero==1,
    ...
    bkgcoef=polyfit([X1;X2],[Y1;Y2],2);  % Fit parabola to sub-group of points
...
plot(xx,yy,'b')
title('iSignal 1.9. Arrow keys to pan and zoom. Press K for keyboard commands')
lyy=min(yy);
...
title('Smooth: S, A/Z    Derivatives: D    PeakSharpen: E,F/V G/B    Peak Meas: P   lin/log: H')
if Sharpen,
    xlabel(['    Smooth: ' SmoothType '    Points: ' num2str(SmoothWidth) '    Ends: ' num2str(ends) '    Der: ' num2str(DerivativeMode)  '    Sharp1: ' num2str(round(10*Sharp1)/10)  '    Sharp2: ' num2str(round(100*Sharp2)/100) ])
else
    xlabel(['    Smooth: ' SmoothType '    Points: ' num2str(SmoothWidth) '    Ends: ' num2str(ends) '    Der: ' num2str(DerivativeMode)  ])    
end
if logymode,
    ylabel('Log y')
else
...
RSquared=cc(2).^2;
   ...
   pos4=.4*yrange;
   pos5=.5*yrange;
   text(center,topaxis(4)-pos1,[' position=' num2str(PeakX)])
   text(center,topaxis(4)-pos2,[' height=' num2str(PeakY)])
   text(center,topaxis(4)-pos3,[' width=' num2str(MeasuredWidth)])
   area=trapz(xx,yy); % Compute the area
   text(center,topaxis(4)-pos4,[' area=' num2str(area) ])
   text(center,topaxis(4)-pos5,[' R2=' num2str(RSquared)])
   if Report,
       disp([PeakX PeakY MeasuredWidth area]);
       Report=0;
   end % Reportpp
...
closestval=x(index);
% ----------------------------------------------------------------------
function Processed=ProcessSignal(x,y,DerivativeMode,w,type,ends,Sharpen,factor1,factor2)
if type==0,w=1;end
if type==4,
    if w<2,w=3;end
